...
export function AssistantInstallPrompt({
  ...
  return (
    <section className="w-full overflow-hidden rounded-[1.55rem] border border-[#a7e3c8] bg-[linear-gradient(180deg,rgba(234,253,245,0.88),rgba(220,248,235,0.7))] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
      <div className="flex items-start gap-3">
        <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-800" />
        <div className="min-w-0 flex-1 space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h3 className="min-w-0 text-sm font-semibold text-emerald-900 [overflow-wrap:anywhere]">{copyText.title}</h3>
            <button
              type="button"
              className="shrink-0 rounded-full border border-[#9ddbbf] bg-white/42 px-3 py-1.5 text-xs font-semibold text-emerald-900 transition hover:bg-white/66"
              onClick={async () => {
                try {
          ...
          <p className="max-w-full break-words text-sm leading-relaxed text-emerald-950/90 [overflow-wrap:anywhere]">
            {prompt}
          </p>
        </div>
      </div>
